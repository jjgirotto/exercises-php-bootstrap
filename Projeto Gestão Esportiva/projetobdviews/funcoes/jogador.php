<?php
    ...
    function buscarJogadores(): array {
        global $pdo;
        $stmt = $pdo->query("SELECT j.*, e.nome as nome_equipe FROM jogador j LEFT JOIN
                                equipe e ON e.id_equipe = j.id_equipe");
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    function buscarJogadorPorId(int $id): ?array {
        global $pdo;
        $statement = $pdo->prepare("SELECT j.*, e.nome as nome_equipe FROM jogador j LEFT JOIN
                                        equipe e ON e.id_equipe = j.id_equipe WHERE j.id_jogador = ?");
